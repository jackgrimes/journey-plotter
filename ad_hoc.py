...
from utils import which_layers_needed, read_in_required_layers, plotter_functions_dict, convert_crop_base_map_layers, \
    get_speeds, scalarMap
from configs import base_layers_configs, map_configs, colored_black_end_points, colored_not_black_end_points, red, \
    data_path, x_lims, y_lims
...
def make_one_off_plots(journeys):
    ...
    for key, value in map_configs.items():
        if value['plotting_or_not']:
            ...
            if key == 'total_fig_no_roads':
                ...
                base_layers['tidal_water'].plot(categorical=False, legend=False, ax=maps_dict[key][1],
                                                color=tuple([x / 255 for x in [0, 0, 60]]), zorder=0)
            else:
                for layer in value['layers']:
                    print('Plotting ' + layer + ' for ' + key)
                    ax.set_facecolor(tuple([x / 255 for x in [232, 237, 232]]))
                    maps_dict[key] = [fig, ax, x_axis, y_axis]
                    base_layers[layer].plot(categorical=False, legend=False, ax=maps_dict[key][1],
                                            color=base_layers_configs[layer][1], zorder=0)
    ...
    for journey in journeys:
                ...
                if ((new_point_gdf.geometry[0].x > x_lims[0]) and
                    ...
                    for key, value in map_configs.items():
                        if value['plotting_or_not']:
                            ...
                            journey_plots_for_moving_recents, journey_plots_for_by_year, journey_plots_shrinking, journey_plots_bubbling = \
                                plotter_functions_dict[key](journey_year_change=False,
                                                            journey_year='',
        ...
        # If plotting fails
            ...
            print("Problem parsing " + (os.path.join(os.path.join(data_path, 'cycling_data'), journey)))
            print(e)
    ...
    for key, value in map_configs.items():
        if value['plotting_or_not']:
            ...
            timestr = journey_str_list2[0] + r"/" + journey_str_list2[1] + r"/" + journey_str_list2[2] + ' to ' + \
                      journey_str_list2[3] + r"/" + journey_str_list2[4] + r"/" + journey_str_list2[5]
            maps_dict[key][0].text(0.0125, 0.025, timestr,
    ...
    for key, value in map_configs.items():
        if value['plotting_or_not']:
            ...
            filename = os.path.join(os.path.join(data_path, 'results'),
                                    runstr + '_ad_hoc_cycling__' + key + '__' + timestr.replace(r'/', '_').replace(' ',
                                                                                                                   '_') + '.png')
...
for this_no_journeys in number_journeys:
    journeys = sorted([x for x in os.listdir(os.path.join(data_path, 'cycling_data')) if '.gpx' in x])[
               -(this_no_journeys):-1]
