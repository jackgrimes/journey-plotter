...
def run():
    ...
    for key, value in map_configs.items():
        if value["plotting_or_not"]:
            ...
            if key in ["running_recents", "by_year"]:
                text_vars[key] = maps_dict[key][0].text(
                    0.9650,
                    0.985,
    ...
    for journey in journeys:
        if attempting_all:
            print(
                ...
                + str(n_journeys_attempted + 1)
                + " of "
                + str(no_journeys)
                + " ("
                + str(round(100 * n_journeys_attempted / no_journeys, 2))
                + "% done)"
            )
            print(
                str(n_journeys_plotted)
                + " journeys were plotted successfully, "
                + str(n_files_unparsable)
                + " journeys were unparsable, "
                + str(n_non_cycling_activities)
                + " journeys were not cycling, "
                + str(n_journeys_outside_London)
            ...
            print(
                "\nAttempting to plot journey number "
                + str(n_journeys_plotted + 1)
                + " of "
                + str(no_journeys)
                + " ("
                + str(round(100 * n_journeys_plotted / no_journeys, 2))
                + "% done)"
            )
            print(
                str(n_journeys_plotted)
                ...
                if (
                    ...
                    if new_journey_year != journey_year:
                        ...
                        if map_configs["by_year"]["plotting_or_not"]:
                            filename = os.path.join(
                                ...
                                runstr + "_cycling_in_year_" + journey_year + ".png",
                            )
                    ...
                    for key, value in map_configs.items():
                        if value["plotting_or_not"]:
                            ...
                            journey_plots_for_moving_recents = returns[
                                "journey_plots_for_moving_recents"
                            ]
                            journey_plots_for_by_year = returns[
                                "journey_plots_for_by_year"
                            ]
                            ...
                            if making_videos:
                                if (n_journeys_plotted + 1) % image_interval == 0:
                                    filename = os.path.join(
                                        ...
                                        "first_"
                                        + str(n_journeys_plotted + 1).zfill(4)
                                        + "_journeys.png",
                                    )
                                    maps_dict[key][0].savefig(
                                        filename,
    ...
    if map_configs["by_year"]["plotting_or_not"]:
        filename = os.path.join(
            os.path.join(data_path, "results"),
            runstr + "_cycling_in_year_" + journey_year + "_to_date.png",
        )
        maps_dict["by_year"][0].savefig(
        ...
        if map_configs["running_recents"]["plotting_or_not"]:
            ...
            while len(journey_plots_for_moving_recents) > 0:
                print(
                    ...
                    + " for the running_recents video - fading out the old journeys"
                )
                ...
                if index % image_interval == 0:
                    filename = os.path.join(
                        ...
                        "first_"
                        + str(n_journeys_plotted + index + 1).zfill(4)
                        + "_journeys.png",
                    )
        ...
        if map_configs["overall_bubbling_off"]["plotting_or_not"]:
            ...
            while len(journey_plots_bubbling) > 0:
                print(
                    ...
                    + " of "
                    + str(number_plots_to_do)
                    + " for the bubbling video - fading out the old journeys"
                )
                # Remove line from plot and journey_plots_for_moving_recents list
                journey_plots_bubbling[0].remove()
                del journey_plots_bubbling[0]
                for i in range(len(journey_plots_bubbling)):
                    journey_plots_bubbling[i].set_sizes(
                        journey_plots_bubbling[i]._sizes * 1.2
                    )
                    if journey_plots_bubbling[i]._alpha > 0.0005:
                        journey_plots_bubbling[i].set_alpha(
                            journey_plots_bubbling[i]._alpha * 0.82
                        )
                # Save the images to files
                if index % image_interval == 0:
                    filename = os.path.join(
        ...
        if map_configs["end_points_bubbling"]["plotting_or_not"]:
            ...
            while len(end_points_bubbling) > 0:
                print(
                    ...
                    + " of "
                    + str(number_plots_to_do)
                    + " for the end_pounts_bubbling video - fading out the old journeys"
                )
                # Remove line from plot and journey_plots_for_moving_recents list
                end_points_bubbling[0].remove()
                del end_points_bubbling[0]
                for i in range(len(end_points_bubbling)):
                    end_points_bubbling[i].set_sizes(
                        end_points_bubbling[i]._sizes * 1.1
                    )
                    if end_points_bubbling[i]._alpha > 0.0005:
                        end_points_bubbling[i].set_alpha(
                            end_points_bubbling[i]._alpha * 0.95
                        )
                # Save the images to files
                if index % image_interval == 0:
                    filename = os.path.join(
        ...
        if map_configs["overall_shrinking"]["plotting_or_not"]:
            ...
            for i in range(number_plots_to_do):
                print(
                    ...
                    + " of "
                    + str(number_plots_to_do)
                    + " for the shrinking video - shrinking the old journeys"
                )
        ...
        if map_configs["end_points_shrinking"]["plotting_or_not"]:
            ...
            for i in range(number_plots_to_do):
                print(
                    ...
                    + " of "
                    + str(number_plots_to_do)
                    + " for the end_points_shrinking video - shrinking the old journeys"
                )
    ...
    file = open(
        os.path.join(os.path.join(data_path, "results"), runstr + "run_notes.txt"), "w"
    )
