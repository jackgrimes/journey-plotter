...
def timesince(time, percent_done):
    ...
    try:
        ...
        return (str(hours) + " hours, " + str(minutes) + " minutes, " + str(seconds) + " seconds taken so far" + "\n" +
                ...
                "Estimated completion time: " + (datetime.datetime.now() + remaining_diff).strftime("%H:%M:%S"))
    except:
...
def make_video(which_set_of_images, runstr, n_journeys_plotted):
    ...
    output = os.path.join(os.path.join(data_path, 'results'),
...
def pack_up_returns(journey_plots_for_moving_recents, journey_plots_for_by_year, journey_plots_shrinking,
                    journey_plots_bubbling, end_points_bubbling, end_points_shrinking):
    return {
...
def plotter_total(inputs):
    ...
    set_of_points.plot(categorical=False,
                       legend=False,
                       ax=ax,
                       color=colors,
                       alpha=0.1,
    ...
    returns = pack_up_returns(journey_plots_for_moving_recents, journey_plots_for_by_year, journey_plots_shrinking,
                              journey_plots_bubbling, end_points_bubbling, end_points_shrinking)
...
def plotter_moving_recents(inputs):
    ...
    journey_plots_for_moving_recents.append(ax.scatter(lngs,
                                                       lats,
                                                       color=colors,
                                                       s=3,
                                                       marker="s",
...
def plotter_by_year(inputs):
    ...
    journey_plots_for_by_year.append(ax.scatter(lngs,
                                                lats,
                                                color=colors,
                                                s=2,
                                                marker="s",
...
def plotter_all_journeys_shrinking(inputs):
    ...
    journey_plots_shrinking.append(ax.scatter(lngs,
                                              lats,
                                              color=colors,
                                              s=50,
                                              # marker="s",
...
def plotter_all_journeys_bubbling_off(inputs):
    ...
    journey_plots_bubbling.append(ax.scatter(lngs,
                                             lats,
                                             color=colors,
                                             s=10,
                                             # marker="s",
...
def plotter_end_points(inputs):
    ...
    lngs = [set_of_points.geometry[0].x, set_of_points.geometry[len(set_of_points.geometry) - 1].x]
    lats = [set_of_points.geometry[0].y, set_of_points.geometry[len(set_of_points.geometry) - 1].y]
...
def plotter_end_points_bubbling_off(inputs):
    ...
    end_points_bubbling.append(ax.scatter(lngs,
                                          lats,
                                          color=['green', 'blue'],
                                          s=10,
                                          # marker="s",
...
def plotter_end_points_shrinking(inputs):
    ...
    end_points_shrinking.append(ax.scatter(lngs,
                                           lats,
                                           color=['green', 'blue'],
                                           s=400,
                                           # marker="s",
        ...
        try:
            base_layers[layer_to_read] = gpd.read_file(
                os.path.join(data_path,
                             os.path.join(r'OS_base_maps/OS OpenMap Local (ESRI Shape File) TQ/data/',
                                          base_layers_configs[layer_to_read][0])))
        except:
            base_layers[layer_to_read] = gpd.read_file(
                os.path.join(data_path,
                             os.path.join(r'OS_base_maps/OS Open Greenspace (ESRI Shape File) TQ/data/',
                                          base_layers_configs[layer_to_read][0])))
    return base_layers


# Put plotter functions in a dictionary to call them more easily
plotter_functions_dict = {'overall': plotter_total,
...
def convert_crop_base_map_layers(base_layers):
    ...
    for key, value in base_layers.items():
        ...
        value['geometry'] = value['geometry'].to_crs({'init': 'epsg:3395'}).cx[x_lims_broader[0]:x_lims_broader[1],
                            y_lims_broader[0]:y_lims_broader[1]]  # Mercator
...
def get_speeds(lats, lngs, times):
    ...
    try:
        ...
        df['distance'] = 2 * R * np.arcsin(((np.sin((df['y2_rad'] - df['y1_rad']) / 2) ** 2) +
                                            (np.cos(df['y1_rad']) * np.cos(df['y2_rad']) *
                                             ((np.sin((df['x2_rad'] - df['x1_rad']) / 2)) ** 2))) ** 0.5)
