...
all_speeds = []
journeys = [x for x in os.listdir(os.path.join(data_path, 'cycling_data')) if '.gpx' in x]
number_to_parse = len(journeys)

for idx, journey in enumerate(journeys):
    print("Parsing journey " + journey + ", number " + str(idx) + " of " + str(number_to_parse) + " (" + str(round(
        100 * (idx - 1) / number_to_parse)) + "% complete)")
    try:
        ...
        if 'cycling' in gpx.tracks[0].name.lower():
            ...
            times = np.array([time.mktime(x['time'].timetuple()) for x in points])  # Extract datetimes in Unix time
